...
// src/index.ts
var COMPETITIONS = {
  WC: { name: "FIFA World Cup 2026", type: "tournament", fdId: 2e3 },
...
var FD_ID_TO_CODE = new Map(
  Object.entries(COMPETITIONS).map(([code, def]) => [def.fdId, code])
);
var STANDINGS_COMPS = ["WC", "PL", "PD", "BL1", "SA", "FL1", "CL"];
function kvScores(comp) {
  return `${comp}:scores`;
}
__name(kvScores, "kvScores");
function kvStandings(comp) {
  return `${comp}:standings`;
}
__name(kvStandings, "kvStandings");
function kvMatch(id) {
  return `match:${id}`;
}
__name(kvMatch, "kvMatch");
function kvSchedule(comp) {
  return `${comp}:schedule`;
}
__name(kvSchedule, "kvSchedule");
var KV_LAST_POLL = "config:last_poll";
var KV_TICK = "config:tick_count";
...
__name(fetchFromFootballData, "fetchFromFootballData");
...
async function fetchFromApiFootball(path, apiKey) {
  return fetch(`${AF_BASE}${path}`, {
    headers: { "x-apisports-key": apiKey }
  });
}
__name(fetchFromApiFootball, "fetchFromApiFootball");
async function findApiFootballFixture(env, compCode, matchDate, homeTla, awayTla) {
  ...
  if (dayCached) {
    ...
    const seasonYear = compCode === "WC" || compCode === "EC" ? season : new Date(matchDate) >= /* @__PURE__ */ new Date(`${season}-07-01`) ? season : season - 1;
  ...
  return fixtures.find((f) => {
    ...
    return (hName.includes(homeUp) || homeUp.includes(hName.slice(0, 3))) && (aName.includes(awayUp) || awayUp.includes(aName.slice(0, 3)));
  }) ?? null;
}
__name(findApiFootballFixture, "findApiFootballFixture");
async function fetchApiFootballDetail(env, fixtureId) {
  ...
  if (fixture) {
    ...
    const ttl = status === "FT" || status === "AET" || status === "PEN" ? 604800 : status === "1H" || status === "2H" || status === "HT" ? 120 : 3600;
    await kvPut(env.SCORES_KV, cacheKey, JSON.stringify(fixture), { expirationTtl: ttl });
  }
  return fixture;
}
__name(fetchApiFootballDetail, "fetchApiFootballDetail");
...
__name(transformMatch, "transformMatch");
...
async function handleCron(env) {
  try {
    ...
    await kvPut(env.SCORES_KV, KV_LAST_POLL, (/* @__PURE__ */ new Date()).toISOString());
  } catch (err) {
...
app.get("/api/:comp/matches", async (c) => {
  ...
  if (cached) {
    const matches2 = JSON.parse(cached);
    const matchday2 = c.req.query("matchday");
    if (matchday2) {
      const md = parseInt(matchday2, 10);
      return c.json({
        matches: matches2.filter((m) => m.matchday === md)
      });
    }
    return c.json({ matches: matches2 });
  }
  const res = await fetchFromFootballData(
...
app.get("/api/match/:id/detail", async (c) => {
  ...
  const compCode = basicData.competition?.code;
  const utcDate = basicData.utcDate;
  const matchDate = utcDate?.slice(0, 10);
  const homeTla = basicData.homeTeam?.tla;
  const awayTla = basicData.awayTeam?.tla;
  if (compCode && matchDate && homeTla && awayTla && c.env.API_FOOTBALL_KEY) {
    try {
      ...
      if (afFixture) {
        const fixtureId = afFixture.fixture?.id;
        if (fixtureId) {
...
// .wrangler/tmp/bundle-MJ9P2D/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-MJ9P2D/middleware-loader.entry.ts
